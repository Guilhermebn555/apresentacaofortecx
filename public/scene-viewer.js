...
export function mountScene(el, slide, component, { root, gateModel, varal }) {
  ...
  const loop = createFrameLoop(time => {
    ...
    // Keep both the mechanical transition and its rain feedback visible so the
    // simulation communicates what the prototype is doing on every machine.
    amount = advanceMotion(amount, active, seconds);
    const raining = !!varal && active === 1;
    if (gateModel) {
